...
func GetComplianceState(policyName string) func() interface{} {
	return func() interface{} {
		rootPlc := utils.GetWithTimeout(ClientHubDynamic, GvrPolicy, policyName, UserNamespace, true, DefaultTimeoutSeconds)
...
func DoCreatePolicyTest(policyFile string, templateGVRs ...schema.GroupVersionResource) {
	...
	if ManuallyPatchDecisions {
		...
		plr := utils.GetWithTimeout(ClientHubDynamic, GvrPlacementRule, plrName, UserNamespace, true, DefaultTimeoutSeconds)
	...
	mplc := utils.GetWithTimeout(ClientManagedDynamic, GvrPolicy, managedPolicyName, ClusterNamespace, true, DefaultTimeoutSeconds)
	...
	for _, tmplGVR := range templateGVRs {
		...
		tmplPlc := utils.GetWithTimeout(ClientManagedDynamic, tmplGVR, policyName, ClusterNamespace, true, DefaultTimeoutSeconds)
...
func DoCleanupPolicy(policyFile string, templateGVRs ...schema.GroupVersionResource) {
	...
	mplc := utils.GetWithTimeout(ClientManagedDynamic, GvrPolicy, managedPolicyName, ClusterNamespace, false, DefaultTimeoutSeconds)
	...
	for _, tmplGVR := range templateGVRs {
		...
		tmplPlc := utils.GetWithTimeout(ClientManagedDynamic, tmplGVR, policyName, ClusterNamespace, false, DefaultTimeoutSeconds)
...
func GetLatestStatusMessage(policyName string, templateIdx int) string {
