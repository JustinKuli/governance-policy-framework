...
var _ = Describe("GRC: [P1][Sev1][policy-grc] Test the policy-rolebinding policy", Ordered, Label("policy-collection", "stable"), func() {
	...
	It("stable/"+policyRoleBindingName+" should be created on the Hub", func() {
		...
		namespace := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: policyRoleBindingNSName, Labels: map[string]string{"e2e": "true"}}}
		...
		_, err = clientHubDynamic.Resource(common.GvrPolicy).Namespace(userNamespace).Patch(
			...
			[]byte(`[{"op": "replace", "path": "/spec/policy-templates/0/objectDefinition/spec/namespaceSelector/include", "value": ["`+policyRoleBindingNSName+`"]}]`),
			metav1.PatchOptions{},
		)
		Expect(err).To(BeNil())
	...
	AfterAll(func() {
		...
		err = clientManaged.CoreV1().Namespaces().Delete(context.TODO(), policyRoleBindingNSName, metav1.DeleteOptions{})
		Expect(err).To(BeNil())
	})
})
