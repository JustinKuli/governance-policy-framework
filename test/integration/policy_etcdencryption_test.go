...
var _ = Describe("GRC: [P1][Sev1][policy-grc] Test the policy-etcdencryption policy", Ordered, Label("policy-collection", "stable", "etcd"), func() {
	...
	It("stable/"+policyEtcdEncryptionName+" should be created on the Hub", func() {
		...
		rootPolicy := utils.GetWithTimeout(
			clientHubDynamic, common.GvrPolicy, policyEtcdEncryptionName, userNamespace, true, defaultTimeoutSeconds,
		)
		...
		_, err := clientHubDynamic.Resource(common.GvrPolicy).Namespace(userNamespace).Patch(
			...
			[]byte(
				`[{"op": "remove", "path": "/spec/remediationAction"},`+
					`{"op": "replace", "path": "/spec/policy-templates/0/objectDefinition/spec/remediationAction", "value": "enforce"}]`,
			),
			metav1.PatchOptions{},
		)
		Expect(err).To(BeNil())
	...
	It("Etcd encryption should be enabled", func() {
		// Only check the first ConfigurationPolicy status since the second one is to inform on the encryption process
